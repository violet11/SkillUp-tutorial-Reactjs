...
const RegisterForm: FC = () => {
  ...
  const onSubmit = handleSubmit(async (data: RegisterUserFields) => {
    ...
    if (response.data?.statusCode === StatusCode.BAD_REQUEST) {
      ...
      if (loginResponse.data?.statusCode === StatusCode.BAD_REQUEST) {
        ...
        // 'avatar' needs to be the same as in backend users.controller.ts (@UseInterceptors(FileInterceptor('avatar', saveImageToStorage))) upload
  ...
  // Every time file changes, useEffect will be run
  useEffect(() => {
    if (file) {
